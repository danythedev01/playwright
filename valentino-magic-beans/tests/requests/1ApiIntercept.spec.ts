...
test("print api calls to products", async ({ page }) => {
  // We must register the event before eveything in order for playwright to listen to it
  // print requests:
...
test("print api calls to products - complete", async ({ page }) => {
  ...
  await page.goto("/products");
  // we'll wait for the moment in which the page won't do any request at all
